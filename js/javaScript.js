...
function addTransactionDOM(transaction) {
    ...
    item.innerHTML = `
    ${transaction.text} <span>${sign2}${Math.abs(transaction.amount)}</span> <button class="del-btn" onclick= "removeTrans(${transaction.id})">x</button>  
      `
...
function update() {
    const amounts = transactions.map(tran => tran.amount)
     ...
     balance_amount = amounts.reduce((accumulator, item) => accumulator += item, 0).toFixed(2)
